...
exports.verifyTokenMetadata = exports.verifyMetadataManifests = exports.verifyConsistentShares = exports.verifyAnimationURL = exports.verifyImageURL = exports.verifyCreatorCollation = exports.verifyAggregateShare = exports.verifyAssets = void 0;
...
const verifyAssets = ({ files, uploadElementsCount }) => {
    const imgFileCount = files.filter(it => {
        return (it.endsWith(constants_2.EXTENSION_PNG) ||
            it.endsWith(constants_2.EXTENSION_JPG) ||
            it.endsWith(constants_2.EXTENSION_GIF));
    }).length;
    const animationFileCount = files.filter(it => {
        return (it.endsWith(constants_3.EXTENSION_MP4) ||
            it.endsWith(constants_3.EXTENSION_MOV) ||
            it.endsWith(constants_3.EXTENSION_MP3) ||
            it.endsWith(constants_3.EXTENSION_FLAC) ||
            it.endsWith(constants_3.EXTENSION_WAV) ||
            it.endsWith(constants_3.EXTENSION_GLB) ||
            it.endsWith(constants_3.EXTENSION_HTML));
    }).length;
    ...
    if (animationFileCount) {
        ...
        if (animationFileCount !== imgFileCount) {
            throw new Error(`number of animation files (${animationFileCount}) is different than the number of img files (${imgFileCount})`);
        }
    }
    ...
    if (animationFileCount) {
        loglevel_1.default.info(`Verifying token metadata for ${jsonFileCount} (img+animation+json) sets`);
    }
    else {
        loglevel_1.default.info(`Verifying token metadata for ${jsonFileCount} (img+json) pairs`);
    }
};
...
const verifyImageURL = (image, files, manifestFile) => {
    ...
    const fileIndex = path_1.default.basename(manifestFile, path_1.default.extname(manifestFile));
...
const verifyAnimationURL = (animation_url, files, manifestFile) => {
    // The animation_url is expected to have the same name as the index
    const fileIndex = manifestFile.split('/').pop().split('.')[0];
    const ext = path_1.default.extname(animation_url);
    const expectedAnimationPath = `${fileIndex}${ext}`;
    if (animation_url !== expectedAnimationPath) {
        // We _could_ match against this in the JSON schema validation, but it is totally valid to have arbitrary URLs to images here.
        // The downside, though, is that those images will not get uploaded to Arweave since they're not on-disk.
        loglevel_1.default.warn(`We expected the \`animation_url\` property in ${manifestFile} to be ${expectedAnimationPath}.
...
The \`metaplex upload\` command will automatically substitute this URL with the Arweave URL location.
    ...
    if (mediaFiles.length === 0 ||
        ...
        throw new Error(`At least one media file entry in \`properties.files\` array is expected to match the \`animation_url\` property.`);
    }
};
exports.verifyAnimationURL = verifyAnimationURL;
...
const verifyMetadataManifests = ({ files }) => {
    ...
    for (const manifestFile of manifestFiles) {
        ...
        if (Object.prototype.hasOwnProperty.call(tokenMetadata, 'animation_url')) {
            ...
            const { animation_url, properties: { files }, } = tokenMetadata;
            (0, exports.verifyAnimationURL)(animation_url, files, manifestFile);
        }
    }
