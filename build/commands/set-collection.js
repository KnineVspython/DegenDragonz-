...
async function setCollection(walletKeyPair, anchorProgram, candyMachineAddress, collectionMint) {
    var _a;
    ...
    if (!collectionMint) {
        ...
        metadataPubkey = await (0, accounts_1.getMetadata)(mintPubkey);
        masterEditionPubkey = await (0, accounts_1.getMasterEdition)(mintPubkey);
        [collectionPDAPubkey] = await (0, accounts_1.getCollectionPDA)(candyMachineAddress);
        [collectionAuthorityRecordPubkey] = await (0, accounts_1.getCollectionAuthorityRecordPDA)(mintPubkey, collectionPDAPubkey);
        signers.push(mint);
        const userTokenAccountAddress = await spl_token_1.Token.getAssociatedTokenAddress(spl_token_1.ASSOCIATED_TOKEN_PROGRAM_ID, spl_token_1.TOKEN_PROGRAM_ID, mint.publicKey, wallet.publicKey);
        instructions.push(...[
        ...
        mintPubkey = await (0, various_1.parseCollectionMintPubkey)(collectionMint, anchorProgram.provider.connection, walletKeyPair);
        metadataPubkey = await (0, accounts_1.getMetadata)(mintPubkey);
        masterEditionPubkey = await (0, accounts_1.getMasterEdition)(mintPubkey);
        [collectionPDAPubkey] = await (0, accounts_1.getCollectionPDA)(candyMachineAddress);
        [collectionAuthorityRecordPubkey] = await (0, accounts_1.getCollectionAuthorityRecordPDA)(mintPubkey, collectionPDAPubkey);
    }
    instructions.push(await anchorProgram.instruction.setCollection({
        accounts: {
