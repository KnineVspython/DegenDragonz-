...
async function getFilePairSize({ image, animation, manifest, }) {
 ...
 */
async function getUpdatedManifest(manifestPath, imageLink, animationLink) {
...
async function processFiles({ signer, filePair, bundlr, storageType, }) {
    ...
    if (filePair.animation) {
        ...
        const animationBuffer = await (0, promises_1.readFile)(filePair.animation);
        if (storageType === storage_type_1.StorageType.ArweaveSol) {
            //@ts-ignore
    ...
    const animationLink = filePair.animation
        ? `https://arweave.net/${animationDataItem.id}?ext=${path_1.default
            ...
            .replace('.', '')}`
        : undefined;
    const manifest = await getUpdatedManifest(filePair.manifest, imageLink, animationLink);
    if (storageType === storage_type_1.StorageType.ArweaveSol) {
        //@ts-ignore
 ...
 */
async function* makeArweaveBundleUploadGenerator(storage, dirname, assets, env, jwk, walletKeyPair, batchSize, rpcUrl) {
    let signer;
    ...
    if (storageType === storage_type_1.StorageType.ArweaveSol) {
        const bytes = (await Promise.all(filePairs.map(getFilePairSize))).reduce((a, b) => a + b, 0);
    ...
    while (filePairs.length) {
        const { count, size } = await getBundleRange(filePairs, storage === storage_type_1.StorageType.ArweaveSol);
        ...
        async function processBundleFilePair(accP, filePair) {
            ...
            const { imageDataItem, animationDataItem, manifestDataItem, arweavePathManifestDataItem, manifest, } = await processFiles({ storageType, signer, bundlr, filePair });
        ...
        if (storageType === storage_type_1.StorageType.ArweaveSol) {
            ...
            let errored = false;
            await promise_pool_1.PromisePool.withConcurrency(batchSize || 20)
                ...
                const uploadTransaction = async () => {
                    await tx.upload().catch(async (err) => {
                        ...
                        await (0, various_1.sleep)(5 * 1000);
...
const withdrawBundlr = async (walletKeyPair) => {
    ...
    if (balance.minus(5000).lte(0)) {
        ...
        try {
            const withdrawResponse = await bundlr.withdrawBalance(balance.minus(5000));
